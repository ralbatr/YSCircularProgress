...
- (void)drawRect:(CGRect)rect {
    ...
    CGPoint centerPoint = CGPointMake(CGRectGetHeight(self.frame)/2, CGRectGetWidth(self.frame)/2);
    // 添加圆到path
    ...
    titleLabel.center = CGPointMake(CGRectGetHeight(self.frame)/2, CGRectGetWidth(self.frame)/3);
    titleLabel.textAlignment = NSTextAlignmentCenter;
    titleLabel.text = self.text;
    titleLabel.font = [UIFont systemFontOfSize:self.radius/4];
    [self addSubview:titleLabel];
    
    UILabel *textLabel = [[UILabel alloc] init];
    textLabel.frame = self.bounds;
    textLabel.center = CGPointMake(CGRectGetHeight(self.frame)/2, CGRectGetWidth(self.frame)*2/3);
    textLabel.textAlignment = NSTextAlignmentCenter;
    textLabel.font = [UIFont systemFontOfSize:self.radius/4];
